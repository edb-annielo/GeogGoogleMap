  ...
  var SAMPLES = 256;
  var ICONS ={
	  A:"http://www.google.com/mapfiles/markerA.png",
	  B:"http://www.google.com/mapfiles/markerB.png",
	  NONE:""
  };
  ...
  var examples = [{
    // Main Sample
    latlngs: [
      [22.388567623110575, 114.32179013573909],
      [22.38122654178802, 114.33535138451839]
    ],
  ...
  function plotElevation(results) {	  
    ...
    for (var i = 0; i < results.length; i++) {
	  var label = '';
	  if(i==0){label='A';}
	  if(i==results.length-1){label='B';}
      data.addRow([label, elevations[i].elevation]);
    }

	/* with box width / height */
  ...
  // Delete a marker and trigger recalculation of the path and elevation
  function delMarker(marker) {
	  	
	marker.setMap(null);
	var pos = jQuery.inArray(marker, markers);
	if(pos >= 0){
  ...
  function updateElevation() {
	//fix icons
	 for (var m in markers) {
	  if(m==0){markers[m].setIcon(ICONS.A);}
	  else if(m==markers.length-1){markers[m].setIcon(ICONS.B);}
	  else{markers[m].setIcon(ICONS.NONE);}
	}  
    if (markers.length > 1) {
